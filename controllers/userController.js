...
const changeStatus = async (req, res) => {
  try {
    ...
    if (user.available === "false") user.available = "true";
    else user.available = "false";

    await user.save();
    return res
      .status(200)
      .json({ message: "status changed successfully", user });
  } catch (err) {
...
const postNotices = async (req, res) => {
  try {
    ...
    if (user.canPost === "false")
      return res.status(200).json({ message: "not eligible to post", user });
    ...
    return res
      .status(200)
      .json({ message: "notice posted successfully", notice });
  } catch (err) {
    return res
      .status(400)
      .json({ message: "error while posting", notice: null });
  }
};
...
const createUser = async (req, res) => {
  ...
  try {
    ...
    // if (!admin)
    //   return res.status(400).json({ message: "not authorised", user: null });

    let isUser = await User.findOne({ empId: req.body.empId });
    if (isUser)
      return res
        .status(200)
