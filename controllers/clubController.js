...
const viewClubLeadDetails = async (req, res) => {
	try{
		let details = await Club.findOne({ _id: req.params.id, leads_id: req.params.clubLead_id })
			.populate('name', 'designation', 'email', 'contactNo', 'cabinLocation', 'available', 'additionalDetails')
			.sort('name');
